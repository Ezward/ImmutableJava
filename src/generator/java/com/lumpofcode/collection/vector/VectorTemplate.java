...
public final class VectorTemplate
{
	...
	public void generateVectorInterface(final Writer writer, final int nodeSize)
	{
		...
		b.line("import com.lumpofcode.annotation.NotNullable;").eol();
		...
		b.block();
		{
			...
			b.line("*/");
			b.line("Vector<T> pushAll(@NotNullable final Iterable<T> iterable);").eol();
			...
			b.line(" */");
			b.line("<R> Vector<R> map(@NotNullable Function<? super T, ? extends R> mapper);").eol();
			...
			b.line(" * @return Vector of elements of type R");
			b.line(" */");
			b.line("<R> Vector<R> flatmap(@NotNullable Function<T, Vector<R>> mapper);");
		}
		b.endBlock();
	...
	public void generateEmptyVector(final Writer writer, final int nodeSize)
	{
		...
		b.line(" * Don't construct this directly, use the Vectors.empty singleton.");
		b.line(" *");
		...
		b.block();
		{
			...
			// set()
			//
			b.line("public Vector<T> set(int index, T value)");
			b.block();
			{
			...
			// push()
			//
			b.line("public Vector<T> push(T value)");
			b.block();
			{
				b.line("return Vectors.asVector(value);");
			}
			b.endBlock();
			...
			b.block();
			{
				b.indent("return Vectors.asVector").args();
				for(int i = 0; i < nodeSize; i += 1)
				{
			...
			// pushAll()
			//
			b.line("public Vector<T> pushAll(final Iterable<T> values)");
			b.block();
			{
	...
	public void generateVectorOfNN(final Writer writer, final int size, final int nodeSize)
	{
		...
		b.block();
		{
			...
			if (size > 0)
			{
				...
				{
					b.arg(i, m.with("i", i).apply("T element{{i}}"));
				}
				b.arg(size - 1, m.apply("T element{{size-1}}"));
			}
			b.endArgs();
			b.eol();
			...
			// get() method
			//
			b.block("public T get(int index)");
			{
			...
			// set() method
			//
			b.block("public Vector<T> set(int index, T value)");
			{
				b.block("switch(index)");
				{
					for (int i = 0; i < size; i += 1)
			...
			// push() method
			//
			b.block("public Vector<T> push(T value)");
			{
				if (size < nodeSize)
				{
			...
			// map() method
			//
			b.line("public <R> Vector<R> map(Function<? super T, ? extends R> mapper)");
			b.block();
			{
			...
			// flatmap() method
			//
			b.line("public <R> Vector<R> flatmap(Function<T, Vector<R>> mapper)");
			b.block();
			{
	...
	public void generateVectorTrie(final Writer writer, final int nodeSize)
	{
		...
		b.block();
		{
			...
			b.block();
			{
				...
				b.line("//");
				b.line("return new VectorTrie(level + 1,this, Vectors.asVector(value));");
			}
			b.endBlock();
			...
			b.block();
			{
				...
				b.block();
				{
					...
					b.line("//");
					b.indent("return new VectorTrie<T>(this.level + 1, this, Vectors.asVector").args();
					for(int i = 0; i < nodeSize; i += 1)
					{
						b.arg(i, m.with("i", i).apply("e{{i}}"));
					}
			...
			b.block();
			{
				b.line("Vector<R> result = Vectors.empty;");
				for(int i = 0; i < nodeSize; i += 1)
				{
	...
	public void generateVectors(final Writer writer, final int nodeSize)
	{
		...
		b.block();
		{
			...
			b.block();
			{
				b.line("return Vectors.empty.pushAll(values);");
			}
			b.endBlock();
			...
			b.line(" * @return Vector of elements of type R");
			b.line(" */");
			b.line("public static <T, R> Vector<R> map(final @NotNullable Vector<T> vector, final @NotNullable Function<? super T, ? extends R> mapper)");
			b.block();
			{
				b.line("//");
				b.line("// do in chunks of node size for efficiency");
			...
			b.line(" * @return Vector of elements of type R");
			b.line(" */");
			b.line("public static <T, R> Vector<R> flatmap(final @NotNullable Vector<T> vector, final @NotNullable Function<T, Vector<R>> mapper)");
			b.block();
			{
				b.line("Vector<R> result = Vectors.empty;");
			...
			b.line(" * @return");
			b.line(" */");
			b.line("public static <T> String toString(final @NotNullable Vector<T> vector)");
			b.block();
			{
